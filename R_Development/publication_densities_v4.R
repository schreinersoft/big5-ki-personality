...
{
  sink(paste("outputs/output_desc_",modelVersion, "_temp", temp, ".txt"))
...
for (facets in facets_list){
  ...
  ggsave(paste("graphics/density_factor_", i, "_", modelVersion, "_temp", temp, "_facets.png"), plot = plots[[i]], dpi=300, width = 8, height = 6)
  i <- i + 1
}
combined_plot <- (plots[[1]] / plots[[2]] / plots[[3]] / plots[[4]] / plots[[5]])
ggsave(paste("graphics/density_", modelVersion, "_temp", temp, "_facets.png"), plot = combined_plot, dpi=300, width = 8, height = 16)
...
save_as_docx(psych_table, path = paste("tables/desc_", modelVersion, "_temp", temp, "_facets.docx"))
...
psych_table <- desc_df %>%
  ...
  autofit() %>%
  align(j = 2:7, align = "center", part = "all")
save_as_docx(psych_table, path = paste("tables/desc_", modelVersion, "_temp", temp, "_factors.docx"))  # !!! ks-werte ergänzen!
...
sink(paste("outputs/output_ANOVA_temperatures_",modelVersion, ".txt"))

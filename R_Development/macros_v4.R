...
for (temp in temperatures)
  ...
  png(paste("graphics/corrplot_", modelVersion, "_temp", temp, ".png"), width = 2400, height = 2400, res = 150)
  ...
  ft <- flextable(fit_data) %>%
    ...
    width(j = 2, width = 1) %>%
    width(j = 3, width = 1.8)
  save_as_docx(ft, path = paste("tables/measures_", modelVersion, "_temp", temp, ".docx"))
  ...
  ggsave(paste("graphics/screeplot_" , modelVersion, "_temp", temp, ".png"), plot = screePlot, dpi=300, width = 8, height = 5)
...
screePlot <- all_scree_data %>% 
  ...
  scale_x_continuous(breaks = 1:length(faModel$values))
screePlot
ggsave(paste("graphics/screeplot_combined_" , modelVersion, ".png"), plot = screePlot, dpi=300, width = 8, height = 5)

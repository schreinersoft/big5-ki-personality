...
def process_openai_v6(batch_size: int, max_num: int, repeats: int=2, service_tier: str = "flex"):
    ...
    with get_session() as db:
        while i < (max_num * repeats):
            ...
            #         .limit(batch_size)\
            #         .all()
            excluded_essays = db.query(OpenAIAnalyzationV6.essay_id)\
                   .subquery()
            excluded_hashes = db.query(OpenAIAnalyzationV6.hash)\
                   .subquery()
            essays = db.query(Essay)\
                    .outerjoin(OpenAIAnalyzationV5)\
...
if __name__ == "__main__":
    process_openai_v6(1, 250, repeats=3) #, service_tier="flex")
